...
var utilGetPrototypeOf =
  Object.getPrototypeOf ||
  function(obj) {
    return obj.__proto__;
  };

export function coreGraph(other, mutable): void {
  ...
  if (other instanceof coreGraph) {
    ...
    this._parentWays = _.assign(
      Object.create(base.parentWays),
      other._parentWays
    );
    this._parentRels = _.assign(
    ...
    for (i = 0; i < keys.length; i++) {
      ...
      if (base.parentWays[child]) {
        for (k = 0; k < base.parentWays[child].length; k++) {
          ...
          if (
            !this.entities.hasOwnProperty(id) &&
