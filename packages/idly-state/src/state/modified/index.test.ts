...
import { virginStateCreate } from '../virgin/index';
...
describe('Get the visible entities', () => {
  describe('series of modified entities addition', () => {
    ...
    it('should add virgins at different quadkeys', () => {
      ...
      expectStable(state.modified).toMatchSnapshot();
    ...
    it('should add related entities, base entities & related entities of base entities', () => {
      ...
      expect(state.modified.has('r1')).toBe(true);
    ...
    it('should add a modified relation', () => {
      ...
      expectStable(state4.modified).toMatchSnapshot();
...
describe('stateAddModifieds', () => {
  ...
  it('saves the base version in modified of modified ids', () => {
    ...
    expectStable(state2.modified.has('w1')).toEqual(false);
  });

  it('should not save the members of modified relation ', () => {
    const baseSetup = () => {
      const s = stateCreate();
      stateAddVirgins(s, [n1, n2, w1], '123');
      stateAddVirgins(s, [r2], '12210');
      stateAddVirgins(s, [w3, n3, n4, n5, n6], '12210');
      return s;
    };
    let state = baseSetup();
    ...
    expect(state.modified.has('w3')).toBe(false);
    expect(state.modified.has('n6')).toBe(false);
  });
...
describe('osmStateShred', () => {
  it('should loose virgin non base ids', () => {
    ...
    expectStable(state.modified).toEqual(
      mapFromObj({
        n1,
        'n1#0': n1Hash0,
      })
