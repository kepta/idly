...
function presetPreset(id, preset, fields) {
    ...
    preset.matchScore = function (tags) {
        /**
    ...
    preset.t = function (scope, options) {
        return t_1.t('presets.presets.' + id + '.' + scope, options);
    };
    ...
    preset.name = function () {
        if (preset.suggestion) {
            id = id.split('/');
            id = id[0] + '/' + id[1];
            return name + ' - ' + t_1.t('presets.presets.' + id + '.name');
        }
     ...
     */
    preset.applyTags = function (tags, geometry, areaKeys) {
        let k;
        tags = _.clone(tags);
        for (k in applyTags) {
            if (applyTags[k] === '*') {
...
exports.presetPreset = presetPreset;
