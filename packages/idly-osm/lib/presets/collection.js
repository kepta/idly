...
const editDistance_1 = require("../helpers/editDistance");
    ...
    const presets = {
        ...
        search(value, geometry) {
            ...
            const similarName = searchable
                ...
                return {
                    preset: a,
                    dist: editDistance_1.utilEditDistance(value, a.name())
                };
            })
                .filter(function (a) {
            ...
            const similarTerms = _.filter(searchable, function (a) {
                return _.some(a.terms() || [], function (b) {
                    return (editDistance_1.utilEditDistance(value, b) + Math.min(value.length - b.length, 0) <
                        3);
                });
            });
            ...
            const similarSuggestions = suggestions
                ...
                return {
                    preset: a,
                    dist: editDistance_1.utilEditDistance(value, suggestionName(a.name()))
                };
            })
                .filter(function (a) {
                return (a.dist +
                    Math.min(value.length - suggestionName(a.preset.name()).length, 0) <
                    1);
            })
                .sort(function (a, b) {
                return a.dist - b.dist;
            ...
            const results = leadingName
                .concat(leadingTerms, leadingTagValues, leadingSuggestions.slice(0, maxSuggestionResults + 5), similarName, similarTerms, similarSuggestions.slice(0, maxSuggestionResults))
