...
import { utilEditDistance } from 'osm/geo_utils/misc';
...
export function presetCollection(collection) {
  ...
  const presets = {
    ...
    item(id) {
      return _.find(this.collection, function(d: any) {
        return d.id === id;
      });
    },
    ...
    search(value, geometry) {
      ...
      const searchable = _.filter(this.collection, function(a: any) {
        return a.searchable !== false && a.suggestion !== true;
      });

      const suggestions = _.filter(this.collection, function(a: any) {
      ...
      const similarTerms = _.filter(searchable, function(a) {
        return _.some(a.terms() || [], function(b: any) {
          return (
