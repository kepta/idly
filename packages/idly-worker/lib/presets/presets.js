...
export function initPresets(d = data.presets) {
    ...
    if (d.defaults) {
        ...
        defaults.set(Geometry.POINT, presetCollection(d.defaults.point.map(getItem)));
        defaults.set(Geometry.VERTEX, presetCollection(d.defaults.vertex.map(getItem)));
        defaults.set(Geometry.RELATION, presetCollection(d.defaults.relation.map(getItem)));
    }
...
export const presets = initPresets();
export const areaKeys = initAreaKeys(presets.all);
export const presetsMatcher = (geometry, tags) => presetsMatch(presets.all, presets.index, areaKeys, geometry, tags);
export const presetsMatcherPoint = weakCache((tags) => presetsMatch(presets.all, presets.index, areaKeys, Geometry.POINT, tags));
export const presetsMatcherVertex = weakCache((tags) => presetsMatch(presets.all, presets.index, areaKeys, Geometry.VERTEX, tags));
export const presetsMatcherLine = weakCache((tags) => presetsMatch(presets.all, presets.index, areaKeys, Geometry.LINE, tags));
export const presetsMatcherAREA = weakCache((tags) => presetsMatch(presets.all, presets.index, areaKeys, Geometry.AREA, tags));
export const presetsMatcherVertexShared = weakCache((tags) => presetsMatch(presets.all, presets.index, areaKeys, Geometry.VERTEX_SHARED, tags));
export const presetsMatcherCached = (geometry) => {
