...
import { getWayGeometry } from 'helpers/getWayGeometry';
...
describe('presetIndex', function () {
    describe('#match', function () {
        ...
        it('returns a collection containing presets matching a geometry and tags', function () {
            ...
            expect(curriedPresetsMatch(getWayGeometry(way), way.tags).id).toEqual('residential');
        });
        it('returns the appropriate fallback preset when no tags match', function () {
            ...
            expect(curriedPresetsMatch(getNodeGeometry(point, new Map()), point.tags).id).toEqual('point');
            expect(curriedPresetsMatch(getWayGeometry(line), line.tags).id).toEqual('line');
        });
        it.skip('matches vertices on a line as vertices', function () {
            ...
            expect(curriedPresetsMatch(getNodeGeometry(point, new Map()), point.tags).id).toEqual('vertex');
        });
        /**
         * @REVISIT when isOnAddressLine is implemented.
         */
        it.skip('matches vertices on an addr:interpolation line as points', function () {
            const presets = initPresets(testPresets);
            const point = nodeFactory({
                id: 'n-1',
                tags: tagsFactory({ leisure: 'park' })
            ...
            expect(curriedPresetsMatch(getNodeGeometry(point, new Map()), point.tags).id).toEqual('park');
        });
    });
    ...
    describe('expected matches', function () {
        ...
        const areaKeys = initAreaKeys(all);
        const curriedPresetsMatch = R.curry(presetsMatch)(all, index, areaKeys);
        // it.skip('prefers building to multipolygon', function() {
        ...
        it('prefers building to address', function () {
            ...
            expect(curriedPresetsMatch(getWayGeometry(way), way.tags).id).toEqual('building');
        });
        it('prefers pedestrian to area', function () {
            ...
            expect(curriedPresetsMatch(getWayGeometry(way), way.tags).id).toEqual('highway/pedestrian');
        });
    });
});
