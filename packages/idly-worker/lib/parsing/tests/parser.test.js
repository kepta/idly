...
const wayXML = '<?xml version="1.0" encoding="UTF-8"?><osm>' +
...
describe('parsers', () => {
    describe('node', () => {
        it('matches snapshot', () => {
            ...
            expect(parseXML(parser.parseFromString(miniXML1, 'text/xml'))).toMatchSnapshot();
        });
    ...
    describe('calculateParentWays ', () => {
        ...
        it('takes existing parentways and adds a way', () => {
            ...
            expect(calculateParentWays(parentWays, [w1]).get('n')).toEqual(new Set([w1.id]));
        });
        ...
        it('takes handles words', () => {
            ...
            expect(calculateParentWays(parentWays, [w1]).get('n-random')).toEqual(new Set([w1.id]));
        });
    });
