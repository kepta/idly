...
import { applyNodeMarkup, DEFAULT_NODE_ICON, nodeCombiner, nodeToPoint } from 'parsing/nodeToFeature';
...
describe('converts node to feat', () => {
    ...
    describe('getNodeGeometry', () => {
        it('should work for empty parentWays', () => {
            expect(getNodeGeometry('n1', new Map())).toEqual(Geometry.POINT);
        });
        ...
        it('should give vertex when inside a way', () => {
            expect(getNodeGeometry('n3780767744', parentWays)).toEqual(Geometry.VERTEX);
        });
        it('should give VERTEX_SHARED when shared between vertices', () => {
            expect(getNodeGeometry('n253179996', parentWays)).toEqual(Geometry.VERTEX_SHARED);
        });
    });
    describe('applyNodeMarkup', () => {
        ...
        it('should get the name', () => {
            const markup = applyNodeMarkup(Geometry.POINT, tagsFactory({ name: 'whatsInTheName' }));
            expect(markup).toEqual({
                icon: DEFAULT_NODE_ICON,
                name: 'whatsInTheName',
                geometry: Geometry.POINT
...
//# sourceMappingURL=nodeToFeature.test.js.map